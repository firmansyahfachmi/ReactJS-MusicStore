        ...
        <div className="card-list" >
            {
                filtered.map(data => (
                    <Link to={`/detail/${data.name}`} style={{ textDecoration: "none" }} key={data.id}>
                        <Card style={{ width: '18rem', border: "none", backgroundColor:"whitesmoke"}} className="card1 shadow p-0 mb-3 rounded" key={data.id}>
                            <h5 style={{margin:10}}>{(data.quantity > 0 ?
                            ...
                            <Card.Body style={{ backgroundColor: "#F5D372", padding: 15, width: "100%" }}>
                                ...
                                {(data.name.length > 40) ? data.name.substr(data.name, 30)+'...' : data.name} </Card.Title>
                            </Card.Body>
                        </Card>
                    </Link>
                ))
